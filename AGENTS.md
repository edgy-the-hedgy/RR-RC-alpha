...
-   **Minimal Churn:** Do not reformat unrelated code, rename adjacent variables, add/remove stray blank lines, or reorder/relocate existing code outside the PR scope — an unreviewable diff is a violation even when the result is functionally identical.
...
-   **Feature Boundaries:** No feature — core or non-core — may depend on a _non-core_ feature (calling its methods, reading its settings, `#include`ing its HLSL, branching on its `IsEnabled()`/`loaded`), guarded or not: a non-core feature can be entirely absent from a given build. Depending on an `IsCore()` feature is fine. Beyond standard registration (`globals::features` in `Globals.h`/`.cpp`) and generic per-feature hook points (`GetActiveConstraints()`, `DrawSettings`, etc.), don't add a branch/field wired to one feature's private need into `Hooks.cpp`/`State.cpp`/`Globals.h`.
...
-   **Minimal Churn:** PRs must touch only the lines required for the change. Do not reformat unrelated code, clean up surrounding structures, rename adjacent variables, or add/remove stray blank lines. Do not reorder or relocate existing functions, types, or members within a file (prefer a forward declaration over moving an existing definition) — a diff that shuffles code around is hard to review even when nothing semantically changed. Extracting a focused helper to satisfy the function-size rule below is not churn; keep such extractions scoped to the function you're already touching, not a drive-by cleanup of untouched code.
...
-   **Single Responsibility:** Each feature class must handle exactly one graphics technique. Break up newly authored C++ functions longer than ~200 lines into focused helper methods; don't refactor an untouched pre-existing function purely to satisfy this limit — that's a minimal-churn violation, not a fix.
-   **Feature Isolation:** Avoid dependency chains onto a feature that isn't guaranteed to ship. No feature — core or non-core — may depend on a _non-core_ feature: calling its methods, reading its settings, `#include`ing its HLSL, or branching on its `IsEnabled()`/`loaded` state, guarded or not. A non-core feature can be entirely absent from a given AIO build (see AIO Bundling Semantics above), so depending on one — even from a core feature — can silently break for users on that build. Depending on an `IsCore()` feature is fine: those are guaranteed to ship. Central engine files (`Hooks.cpp`, `State.cpp`/`.h`, `Globals.h`) stay feature-agnostic beyond the registration every feature already needs (`globals::features` declarations) and generic per-feature hook points already provided for exactly this purpose — e.g. `Feature::GetActiveConstraints()` for declaring cross-feature setting conflicts, or `DrawSettings`/`SetupResources`/`Draw`/`Prepass` for rendering. Don't add a branch, field, or hook wired to one specific feature's private need. If a technique needs a new engine interception point, expose it the same way — a virtual/override any feature could use (see `docs/new-feature-template/`) — not code that names one feature by hand.
...
-   **VR-Only Files:** The minimal-divergence rule applies to code shared with, forked from, or synced against upstream's non-VR path. A file counts as VR-only only when it is compiled/loaded exclusively for VR, has no SE/AE runtime path, and is not a fork of an upstream shared file — this exemption relaxes _only_ the requirement to mirror non-VR structure. Every other rule in this document still applies to a VR-only file: DRY/shared-utility reuse, `Util::SetResourceName`, `CS_GPU_PASS` instrumentation, comment standards, and single-responsibility/function-size limits.
...
-   **Feature Workflow:** Start from `docs/new-feature-template/`, implement the C++ class inheriting from `Feature` (`DrawSettings()`, `LoadSettings()`, `SaveSettings()`, and feature-specific rendering hooks), and register in appropriate source files and `globals::features`.
