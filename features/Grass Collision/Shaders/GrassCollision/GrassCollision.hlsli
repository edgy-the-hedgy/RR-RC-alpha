...
namespace GrassCollision
{
		...
		float nearFactor, float4x4 worldMatrix, out float3 bendAxis, out float bendAngle)
	{
		...
		bendAngle = 0.0;
		if (worldBendMagnitude > EPSILON_WIND_RESPONSE && bendWeight > EPSILON_WIND_RESPONSE && nearFactor > EPSILON_WIND_RESPONSE) {
			...
			if (modelDirectionLength > EPSILON_WIND_RESPONSE) {
		...
		out float3 bendAxis, out float bendAngle)
	{
		...
		float bladeHeight = vertexHeight / max(normalizedHeight, EPSILON_WIND_HEIGHT);
		...
		out float3 bendAxis, out float bendAngle)
	{
		...
		currentField.w = smoothstep(FADE_DISTANCE, 0.0, length(currentRootWorld));
		previousField.w = smoothstep(FADE_DISTANCE, 0.0, length(previousRootWorld));
