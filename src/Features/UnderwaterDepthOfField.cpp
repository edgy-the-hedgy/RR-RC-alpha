...
namespace
{
	...
	bool EnsureScratchTarget(RE::BSGraphics::RenderTargetData& a_target, ScratchTarget& a_scratch, const char* a_name)
	{
		...
		a_target.texture->GetDesc(Util::AsW32(&desc));
	...
	bool DrawDepthOfFieldInputPass(RE::BSGraphics::RenderTargetData& a_target, ScratchTarget& a_scratch, ID3D11ShaderResourceView* a_maskSRV)
	{
		...
		context->CopyResource(a_scratch.texture->resource.get(), Util::AsReal(a_target.texture));
		...
		ID3D11ShaderResourceView* srvs[] = {
			...
			Util::AsReal(globals::game::renderer->GetDepthStencilData().depthStencils[RE::RENDER_TARGETS_DEPTHSTENCIL::kMAIN].depthSRV),
			a_maskSRV
		};
		...
		ID3D11RenderTargetView* rtvs[] = { Util::AsReal(a_target.RTV) };
	...
	RE::BSGraphics::RenderTargetData* FindDepthOfFieldInputTarget(ID3D11ShaderResourceView* a_sourceSRV)
	{
		...
		for (auto targetIndex : kDepthOfFieldInputTargets) {
			...
			if (Util::AsReal(target.SRV) == a_sourceSRV || Util::AsReal(target.texture) == sourceResource) {
				result = &target;
				break;
			}
		}
	...
	void ApplyUnderwaterFogToDepthOfFieldInput(RE::ImageSpaceEffectParam* a_downsampleParam)
	{
		...
		auto* maskSRV = currentOptions.masked ? Util::AsReal(underwaterMask.SRV) : nullptr;
