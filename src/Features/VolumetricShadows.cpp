...
void VolumetricShadows::CopyShadowLightData()
{
...

	{
		...
		if (shadowView) {
			...
			shadowView->GetResource(Util::AsW32(&shadowResource));
			...
			if (shadowResource) {
				...
				if (shadowTexture) {
					if (downsampleMip0 && downsampleMip1 && blurHorizontal && blurVertical) {
						...
						ID3D11ShaderResourceView* csSrvs[2]{ Util::AsReal(shadowView), Util::AsReal(esramDepthStencil.depthSRV) };
		...
		auto* srv = shadowView ? (shadowCopyUpdated && shadowCopySRV ? shadowCopySRV : Util::AsReal(shadowView)) : nullptr;
