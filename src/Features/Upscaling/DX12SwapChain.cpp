...
HRESULT DX12SwapChain::Present(UINT SyncInterval, UINT Flags)
{
	...
	// is covered by the D3D11â†’D3D12 fence. Without this, FidelityFX may read
...
WrappedResource::WrappedResource(D3D11_BUFFER_DESC a_bufferDesc, ID3D11Device5* a_d3d11Device, ID3D12Device* a_d3d12Device, const std::string& a_name)
{
    // Buffer counterpart of the existing shared-texture path.
    // D3D11 owns the allocation; D3D12 opens the same NT-shared resource.
    // Structured buffers shared with D3D12 use the NT-handle path.
    // Do not combine D3D11_RESOURCE_MISC_SHARED with SHARED_NTHANDLE here;
    // the legacy shared flag makes this structured-buffer descriptor invalid.
    a_bufferDesc.MiscFlags |=
        D3D11_RESOURCE_MISC_SHARED_NTHANDLE;

    auto throwIfFailed = [&](HRESULT a_result, const char* a_operation) {
        if (FAILED(a_result)) {
            logger::error(
                "[DX12SwapChain] Wrapped buffer '{}' {} failed: HRESULT 0x{:08X}, byte width {}, bind flags 0x{:X}, misc flags 0x{:X}, stride {}",
                a_name.empty() ? "<unnamed>" : a_name.c_str(),
                a_operation,
                static_cast<uint32_t>(a_result),
                a_bufferDesc.ByteWidth,
                a_bufferDesc.BindFlags,
                a_bufferDesc.MiscFlags,
                a_bufferDesc.StructureByteStride);
        }
    ...
    if (!a_name.empty())
        Util::SetResourceName(buffer11, "%s", a_name.c_str());

    winrt::com_ptr<IDXGIResource1> dxgiResource;

    throwIfFailed(
        buffer11->QueryInterface(
    ...
    const HRESULT openResult =
        a_d3d12Device->OpenSharedHandle(
            sharedHandle,
    ...
    if (a_bufferDesc.BindFlags & D3D11_BIND_SHADER_RESOURCE) {

        if (a_bufferDesc.StructureByteStride == 0)
            throw std::runtime_error(
                "Wrapped structured buffer has zero StructureByteStride");
        ...
        throwIfFailed(
            a_d3d11Device->CreateShaderResourceView(
                buffer11,
                &srvDesc,
                &srv),
    ...
    if (a_bufferDesc.BindFlags & D3D11_BIND_UNORDERED_ACCESS) {
        ...
        throwIfFailed(
            a_d3d11Device->CreateUnorderedAccessView(
                buffer11,
                &uavDesc,
                &uav),
