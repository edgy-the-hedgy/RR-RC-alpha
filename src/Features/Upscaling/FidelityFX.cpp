...
void FidelityFX::Present(bool a_useFrameGeneration, bool a_isHDR)
{
	...
	if (a_useFrameGeneration) {
		...
		configParameters.frameGenerationCallback = [](ffxDispatchDescFrameGeneration* params, void* pUserCtx) -> ffxReturnCode_t {
			// The stock swapchain requests one output. The native-x3 provider fork requests
			// two outputs in the same dispatch. Log the first observed count so a test run
			// immediately proves which provider path is active.
			static std::atomic<uint32_t> observedGeneratedFrameCount{ 0 };
			uint32_t expected = 0;
			if (observedGeneratedFrameCount.compare_exchange_strong(expected, params->numGeneratedFrames))
				logger::info("[FidelityFX] Frame-generation callback requested {} generated frame(s)", params->numGeneratedFrames);
