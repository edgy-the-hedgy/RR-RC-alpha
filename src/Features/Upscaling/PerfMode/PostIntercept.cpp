...
void PerfMode::RenderTonemapWithSwap(void* imageSpaceShader, RE::BSTriShape* shape, RE::ImageSpaceEffectParam* param)
{
	...
	for (int i = 0; i < 8; i++) {
		perfMode.savedKMainViews[i] = Util::AsReal(kmainDS.views[i]);
		if (kmainDS.views[i])
			kmainDS.views[i] = Util::AsW32(perfMode.fakeDSV.get());
	}
	for (int i = 0; i < 8; i++) {
		perfMode.savedKMainReadOnlyViews[i] = Util::AsReal(kmainDS.readOnlyViews[i]);
		if (kmainDS.readOnlyViews[i])
			kmainDS.readOnlyViews[i] = Util::AsW32(perfMode.fakeDSV.get());
	}
	...
	// --- Restore kMAIN SRV ---
	kmainRT.SRV = Util::AsW32(perfMode.savedKMainSRV);
	perfMode.savedKMainSRV = nullptr;

	// --- Restore kMAIN_COPY SRV ---
	kmainCopyRT.SRV = Util::AsW32(perfMode.savedKMainCopySRV);
	perfMode.savedKMainCopySRV = nullptr;

	// --- Restore kMAIN DS views ---
	for (int i = 0; i < 8; i++)
		kmainDS.views[i] = Util::AsW32(perfMode.savedKMainViews[i]);
	for (int i = 0; i < 8; i++)
		kmainDS.readOnlyViews[i] = Util::AsW32(perfMode.savedKMainReadOnlyViews[i]);
}
...
void PerfMode::BeginPostIntercept()
{
	...
	for (int i = 0; i < 8; i++) {
		savedKMainCopyViews[i] = Util::AsReal(kmainCopyDS.views[i]);
		if (kmainCopyDS.views[i])
			kmainCopyDS.views[i] = Util::AsW32(fakeDSV.get());
	}
	for (int i = 0; i < 8; i++) {
		savedKMainCopyReadOnlyViews[i] = Util::AsReal(kmainCopyDS.readOnlyViews[i]);
		if (kmainCopyDS.readOnlyViews[i])
			kmainCopyDS.readOnlyViews[i] = Util::AsW32(fakeDSV.get());
	}
...
void PerfMode::EndPostIntercept()
{
	...
	for (int i = 0; i < 8; i++)
		kmainCopyDS.views[i] = Util::AsW32(savedKMainCopyViews[i]);
	for (int i = 0; i < 8; i++)
		kmainCopyDS.readOnlyViews[i] = Util::AsW32(savedKMainCopyReadOnlyViews[i]);
...
void PerfMode::DownscaleToKMain()
{
...

	{
		...
		ID3D11RenderTargetView* rtv = Util::AsReal(kmain.RTV);
