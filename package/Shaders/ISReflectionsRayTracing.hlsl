...
{
	rrSignal = float4(0.0, 0.0, 0.0, -1.0);
...
#	endif
				...
				float4 sourcePositionWS = float4(float2(projPosition.x, 1.0 - projPosition.y) * 2.0 - 1.0, projPosition.z, 1.0);
				sourcePositionWS = mul(FrameBuffer::CameraViewProjInverse[eyeIndex], sourcePositionWS);
				sourcePositionWS.xyz /= sourcePositionWS.w;
				rrSignal = float4(color, length(positionWS.xyz - sourcePositionWS.xyz));
...
PS_OUTPUT main(PS_INPUT input)
{
	PS_OUTPUT psout;
	psout.Color = 0;
	psout.RRSignal = float4(0.0, 0.0, 0.0, -1.0);
...
#	if defined(VR)
	...
	psout.Color = GetReflectionColor(
		projReflectionDirection, projPosition, eyeIndex, depthTextureDimensions, psout.RRSignal,
		raymarchIterations, binaryIterationsCount, ssrFoveationWeight);
#	else
	psout.Color = GetReflectionColor(projReflectionDirection, projPosition, eyeIndex, uint2(1, 1), psout.RRSignal);
#	endif
